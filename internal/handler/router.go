...
func NewRouter(cfg *config.Config, logger applog.Logger, deps Dependencies) *gin.Engine {
	...
	venues := adminJWT.Group("/venues")
	{
		...
		venues.POST("/:id/surveys/:surveyID/duplicate", surveyH.Duplicate)
		venues.GET("/:id/surveys/:surveyID/stats", surveyH.Stats)
		venues.GET("/:id/surveys/:surveyID/participants", surveyH.ListParticipants)
		venues.GET("/:id/surveys/:surveyID/export-csv", surveyH.ExportCSV)
